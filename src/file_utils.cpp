...
[[nodiscard]] std::optional<std::vector<unsigned char>> sha256_bytes_of_stream(std::istream& is) {
    ...
    status = BCryptGetProperty(hAlg, BCRYPT_OBJECT_LENGTH, reinterpret_cast<PUCHAR>(&cbHashObject), sizeof(DWORD),
                               &cbData, 0);
    if (status != 0) {
...
[[nodiscard]] std::optional<std::string> sha256_file(const std::filesystem::path &p) {
    if (std::error_code ec; !std::filesystem::exists(p, ec)) return std::nullopt;
...
[[nodiscard]] bool clear_directory(const std::filesystem::path &dir) {
    ...
    for (auto &entry: std::filesystem::directory_iterator(dir, ec)) {
        std::error_code ec2;
        std::filesystem::remove_all(entry, ec2);
...
[[nodiscard]] std::wstring get_temp_file_guid() {
    ...
    if (const HRESULT hr = CoCreateGuid(&g); FAILED(hr)) {
        ...
        const std::filesystem::path p = std::filesystem::path(tmpDir) / name;
        return p.wstring();
    }
    wchar_t name[64];
