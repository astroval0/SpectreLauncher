...
3) it checks if the game's BEClient_x64.dll matches the one we need (if not it will clear that directory and download the one we need there. the code for that dll is also open source)
